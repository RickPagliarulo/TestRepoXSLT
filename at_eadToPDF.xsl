        ...
        <fo:root xmlns:fo="http://www.w3.org/1999/XSL/Format">
            ...
            <fo:layout-master-set>
                ...
                <fo:simple-page-master master-name="cover-page" page-width="8.5in"
                    page-height="11in" margin-top="0.1in" margin-bottom="0.3in" margin-left="0.5in" margin-right="0.5in">
                ...
                <fo:simple-page-master master-name="toc" page-width="8.5in" page-height="11in"
                    margin-top="0.1in" margin-bottom="0.3in" margin-left="0.5in" margin-right="0.5in">
   ...
   <!--xxxxFooterxxxx-->
            ...
            <fo:page-sequence master-reference="toc">                                               
                ...
                <fo:table>
                    ...
                    <fo:table-body>
                        <fo:table-row>
                            <fo:table-cell>
                                <fo:block color="black" font-size="10pt" text-align="left" font-family="Times"> 
                                    <xsl:value-of select="$pageHeader"/>
                                </fo:block>
                            </fo:table-cell>
                                              
                            <fo:table-cell>    
                                <fo:block text-align="right" color="black" font-size="10pt" font-family="Times">
    ...
    <xsl:template match="ead:eadheader">
        <fo:block text-align="center" padding-top=".5in" font-weight="bold"
            line-height="24pt" space-after="18pt" 
            padding-bottom="12pt">
                ...
                <xsl:choose>
                    <xsl:when test="ead:filedesc/ead:titlestmt/ead:titleproper">                                        
                        <xsl:choose>
                            ...
                            <xsl:otherwise><xsl:apply-templates select="ead:filedesc/ead:titlestmt/ead:titleproper/text()"/></xsl:otherwise>
                        </xsl:choose>
                    </xsl:when>
                    <xsl:otherwise><xsl:apply-templates select="/ead:ead/ead:archdesc/ead:did/ead:unittitle" mode="header"/></xsl:otherwise>                
                </xsl:choose>
    ...
    <xsl:template match="ead:publicationstmt">
       <fo:block font-family="Times"> 
        ...
        <fo:block><xsl:text>John J. Burns Library</xsl:text></fo:block>
    ...
    <xsl:template match="ead:archdesc">
        ...
        <xsl:if
            ...
            <xsl:apply-templates select="/ead:ead/ead:eadheader/ead:filedesc/ead:titlestmt" mode="admin"/>
        ...
        <xsl:if test="/ead:ead/ead:archdesc/ead:bioghist">
            <fo:block page-break-before="always"/>   <!-- RP inserted this line to start the biographical note information on a new page -->
        ...
        <xsl:if test="/ead:ead/ead:archdesc/ead:scopecontent">
            <fo:block page-break-before="always"/>  <!-- RP inserted this line to start the scope and content on a new page; ideally would have a conditional formatting based on position on the page (i.e., close to the bottom of the page = insert page break, but not sure if XSL can do that -->
                    ...
                    <xsl:choose>
                        ...
                        <xsl:otherwise>
                            <xsl:choose>
                                ...
                                <xsl:when test="self::ead:unitdate">Date
                                    <xsl:if test="@type">
    ...
    <xsl:template match="/ead:ead/ead:eadheader/ead:revisiondesc" mode="admin">
        <fo:block font-size="12pt" space-before="18pt" space-after="8pt"
            margin-left="-4pt" font-weight="bold" color="#111">Last Update</fo:block>
        <fo:block>
    ...
    <xsl:template match="ead:archdesc/ead:dsc">
        <xsl:choose>
            ...
            <xsl:otherwise>
                <fo:block font-size="16pt" space-before="36pt" space-after="14pt"
        ...
        <fo:table table-layout="fixed" space-after="16pt" space-before="16pt" width="7.25in" font-size="12pt" line-height="18pt" border-top="1pt solid #000" border-bottom="1pt solid #000">
    ...
    <xsl:template name="clevel">
        ...
        <xsl:for-each select=".">
            <xsl:choose>
                ...
                <xsl:when test="@level='series' or @level='subseries'">                      
                    <fo:table-row background-color="{$colorClass}" border-bottom="1pt solid #fff">
                        <xsl:choose>
                            ...
                            <!-- RP trying to make background color less dark below: Switched it to #fff -->
                            
                            
                            <xsl:otherwise><xsl:attribute name="background-color">#fff</xsl:attribute></xsl:otherwise>
                        </xsl:choose>
                        <xsl:choose>
                ...
                <xsl:when test="@level='subcollection' or @level='subgrp' or @level='collection'or @level='fonds' or 
                ...
                <xsl:when test="count(child::*/ead:container/@id) &gt; 1">
                    <fo:table-row background-color="{$colorClass}" border-bottom="1pt solid #fff">
                        <!-- RP changing padding from 8pt and padding-before from 8pt and adding border-top for a test -->
                        <fo:table-cell number-columns-spanned="5" padding="3pt" border-top="1pt solid gray" border-bottom="1pt solid gray">
                            <fo:block padding-before="0pt">  <!-- RP changed the padding for this from 4pt to 0pt -->
                                <!-- RP hiding <xsl:apply-templates select="ead:did" mode="dsc"/> to remove the big header, and insert it below -->
                                <!-- RP changing line-height from 18pth to something smaller
                                    and also changing font-size from 12pt to something a little smaller
                                    and also changing "space before" from 36pt to something smaller
                                    and also changing border-top and -bottom colors from 666 to CCC -->
                                <fo:table table-layout="fixed" space-after="24pt" 
                                    space-before="18pt" width="6in" font-size="10pt" 
                                    line-height="14pt" margin-left="18pt" padding="0in" 
                                    >
                                   ...
                                   <!--    <fo:table-row>
                                            <fo:table-cell width="3.0in">
                                                <xsl:apply-templates select="ead:did" mode="dsc"/>
                                            </fo:table-cell> -->
                                            
                                        
                                        
                                        <!-- RP For the below for-each statement, this formats the FIRST entry
                                            for an item with MULTIPLE folder and box locations
                                            -->
                                        <xsl:for-each select="child::*/ead:container[@label][1]">   
                                        ...
                                        <!--     <fo:table-row>
                                            ...
                                            <fo:table-row>
                                               <fo:table-cell width="3.5in">
                                                       <!-- <xsl:value-of select="@label"/>-->
                                                        <xsl:apply-templates select="ancestor-or-self::ead:did" mode="dsc"/>    
                                                </fo:table-cell> 
                                                ...
                                                <fo:table-cell> 
                                                    <xsl:choose>
                                                        <xsl:when test="$container = ''"><xsl:attribute name="number-columns-spanned">1</xsl:attribute></xsl:when> 
                                                        <xsl:when test="$container = 1"><xsl:attribute name="number-columns-spanned">1</xsl:attribute></xsl:when>
                                                        <xsl:when test="$container = 2"><xsl:attribute name="number-columns-spanned">1</xsl:attribute></xsl:when>
                                                        <xsl:when test="$container = 3"><xsl:attribute name="number-columns-spanned">2</xsl:attribute></xsl:when>
                                                        <xsl:when test="$container = 4"/>
                                                        <xsl:otherwise/>
                                                    </xsl:choose>
                                        ...
                                        <!--RP For the below for-each statement, this formats each of the SUBESEQUENT
                                            folder and box locations for an item that has multiple locations
                                            -->
                                        <xsl:for-each select="child::*/ead:container[@label][2] | child::*/ead:container[@label][3] | child::*/ead:container[@label][4] |
                                            child::*/ead:container[@label][5] | child::*/ead:container[@label][6] | child::*/ead:container[@label][7]">
                                            <xsl:variable name="id" select="@id"/>
                                            <xsl:variable name="container" select="count(../ead:container[@parent = $id] | ../ead:container[@id = $id])"/>
                                            <!--     <fo:table-row>
                                                <fo:table-cell>
                                            ...
                                            <xsl:for-each select="descendant-or-self::ead:dao">
                                                <fo:table-row>
                                                    <fo:table-cell number-columns-spanned="4">
                                                        <!-- RP changed padding-before from 8pt to 0pt -->      
                    ...
                    <fo:table-row background-color="{$colorClass}" border-bottom="1pt solid #fff">
                        ...
                        <fo:table-cell number-columns-spanned="5" padding="3pt" border-top="1pt solid gray" border-bottom="1pt solid gray">
                            ...
                            <fo:table table-layout="fixed" space-after="24pt" 
                                  ...
                                  <fo:table-row>
                                        <fo:table-cell width="3.5in">
                                          ...
                                          <xsl:for-each select="child::*/ead:container[@label]">
                                              ...
                                              <fo:table-cell> 
                                                  <xsl:choose>
                                                      <xsl:when test="$container = ''"><xsl:attribute name="number-columns-spanned">1</xsl:attribute></xsl:when> <!-- RP 4, 4, 3, 2 -->
                                                      <xsl:when test="$container = 1"><xsl:attribute name="number-columns-spanned">1</xsl:attribute></xsl:when>
                                                      <xsl:when test="$container = 2"><xsl:attribute name="number-columns-spanned">1</xsl:attribute></xsl:when>
                                                      <xsl:when test="$container = 3"><xsl:attribute name="number-columns-spanned">2</xsl:attribute></xsl:when>
                                                      <xsl:when test="$container = 4"/>
                                              ...
                                              <!--RP For the below for-each statement, this formats box/folder listings for ALL ITEMS AFTER THE FIRST ITEM
                             ...
                             <!-- RP Here is the stuff that says "digital object" and shows the link and all that business -->
                                    <xsl:if test="descendant-or-self::ead:dao">
                                        <xsl:for-each select="descendant-or-self::ead:dao">
                                            <fo:table-row>
                                                <fo:table-cell number-columns-spanned="4">
                ...
                <xsl:when test="@level='file' or @level='item' or (@level='otherlevel'and child::ead:did/ead:container)">
                  ...
                  <fo:table-row background-color="{$colorClass}" border-bottom="1pt solid #fff">
                        <fo:table-cell padding="8pt" padding-left="{$clevelMargin}">
                            <xsl:choose>
                                <xsl:when test="count(ead:did/ead:container) &lt; 1">
                                    <xsl:attribute name="number-columns-spanned">
                                        <xsl:text>5</xsl:text>
        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                ...
                <xsl:if test="ead:unittitle/text() or ead:unittitle/ead:title/text()">
                    <xsl:text>, </xsl:text>  <!--RP changed this from &#160; to a comma with a space to separate unit title from unit date a little more -->
                </xsl:if>
